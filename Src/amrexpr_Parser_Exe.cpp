                         ...
                         std::vector<char const*>& local_variables)
{
    ...
    case PARSER_F3:
    {
        AMREXPR_ALWAYS_ASSERT_WITH_MESSAGE(((struct parser_f3*)node)->ftype == PARSER_IF,
                      ...
                      std::vector<char const*> const& locals)
{
    ...
    {
        if (i >= AMREXPR_PARSER_LOCAL_IDX0) {
            return {locals[i-AMREXPR_PARSER_LOCAL_IDX0],paren_atom};
        } else {
    ...
    while (*((parser_exe_t*)p) != PARSER_EXE_NULL) { // NOLINT
        ...
        case PARSER_EXE_SYMBOL:
        {
            ...
            os << std::setw(3) << count++
               ...
               << pstack.back().first;
            if (i >= AMREXPR_PARSER_LOCAL_IDX0) {
                os << " = " << pstack[i-AMREXPR_PARSER_LOCAL_IDX0].first;
            }
            os << "\n";
            p += sizeof(ParserExeSymbol);
            break;
        ...
        default:
            AMREXPR_ALWAYS_ASSERT_WITH_MESSAGE(false,"parser_print: unknown node type");
        }
    }
}
